...
$TransactionType = $_GET['typeoftransaction'];
$Where = "";
...
if ($TransactionType != "All") {
    $TransactionType = $_GET['typeoftransaction'];
    $Where = 'store_id = "'.$storeid.'" AND zreading >= "'.$Date1.'" AND zreading <= "'.$Date2.'" AND transaction_type = "'.$TransactionType.'" ';
} else {
    $Where = 'store_id = "'.$storeid.'" AND zreading >= "'.$Date1.'" AND zreading <= "'.$Date2.'" ';
}
...
echo json_encode(
    ...
    SSP::complex ($_GET, ConnectionArray(), $table, $primaryKey, $columns, $whereResult=null, $whereAll=$Where)
);
