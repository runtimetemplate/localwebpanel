...
if ($_GET['storeid'] == 'All') {
	if ($_GET['btnid'] == 2) { 
		...
		$query = query("SELECT sum(grosssales), store_id FROM posrev.admin_daily_transaction WHERE guid = '".$_SESSION['client_user_guid']."' AND zreading >= '".$Date1."' and zreading <= '".$Date2."' group by store_id order by grosssales asc;");
		confirm($query);
		while ($row = fetch_array($query)) { 
