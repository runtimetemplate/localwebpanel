...
if ($_GET['storeid'] == 'All') {
	...
	$query = query("SELECT store_id, zreading, sum(quantity) as qty , sum(total) as totalsales  FROM posrev.admin_daily_transaction_details WHERE store_id = '".$_GET['storeid']."' AND zreading >= '".$Date1."' and zreading <= '".$Date2."' AND product_id = ".$productID." group by zreading order by zreading asc");
